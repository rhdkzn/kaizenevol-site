 ...
 *
 * One row per prospect in public.ke_onboarding, keyed by an unguessable token that IS
 * the link. NO SERVICE-ROLE KEY IS NEEDED: the public page reaches its own row only
 * through three security-definer functions (onboard_peek / onboard_get / onboard_sign),
 * called with the anon key and the token. Writes that need trust — marking paid and
 * provisioning the client — are done by the authenticated CRM, not here.
 *
 *   GET  /api/onboard?op=get&t=<token>       the funnel as the prospect may see it (marks viewed)
 *   POST /api/onboard?op=sign&t=<token>      { name, image, agreed, hash } → signature stored, emails sent
 *   GET  /api/onboard?op=status&t=<token>    current status; if STRIPE_SECRET_KEY is set, also
 *                                            whether Stripe holds a paid session for this token
...
async function rpc(name, args) {
  ...
  if (!r.ok) { const t = await r.text().catch(() => ''); throw new Error('db ' + r.status + ' ' + t.slice(0, 200)); }
  return await r.json();
}
...
export function agreementText(d) {
  ...
  if (d.founding) L.push(`2.4 The founding rate is a discount for one of the Agency's first five clients. It ends at the first growth step under clause 3, at which point the retainer becomes the standard rate of ${gbp(TERMS.standard)} per month in place of the ${gbp(step)} increase in clause 3.2; any later step then applies from the standard rate.`);
  L.push(``);
  ...
  L.push(`3.5 When a step is verified the Agency notifies the Client in writing with the figures relied on. The Client authorises the Agency to increase the recurring charge to the new rate from the next billing date after that notice, and to charge the one-off bonus at the same time. If the Client disputes the figures it must say so in writing within 7 days; the step then takes effect from the next billing date after the dispute is resolved.`);
  L.push(``);
  ...
  L.push(`8.1 Where the Agency processes personal data of the Client's customers (for example to send email or SMS, or to run advertising), the Client is the controller and the Agency the processor under the UK GDPR and the Data Protection Act 2018. Subject matter and duration: the services in clause 1 for the term of this Agreement. Nature and purpose: planning and sending marketing email and SMS, running advertising campaigns, and measuring their results. Personal data: names, contact details, purchase and engagement data of the Client's customers and subscribers. Data subjects: the Client's customers, subscribers and audience.`);
  L.push(`8.2 The Agency will: (a) process the personal data only on the Client's documented instructions, including this Agreement; (b) ensure the people it authorises to process it are bound by confidentiality; (c) apply appropriate technical and organisational security measures (UK GDPR Article 32); (d) engage sub-processors only under the Client's general authorisation and on equivalent terms, remaining responsible for them — at signing these are Meta, Klaviyo, Shopify, Stripe, Resend and Supabase, and the Agency will tell the Client before adding another; (e) assist the Client with data-subject rights requests; (f) assist the Client with security, breach notification and impact assessments (Articles 32 to 36) and notify the Client without undue delay on becoming aware of a personal-data breach; (g) at the Client's choice, delete or return the personal data when the services end; and (h) make available the information needed to demonstrate compliance and allow reasonable audits.`);
  L.push(`8.3 The Client warrants that it has a lawful basis, and any consents required under the Privacy and Electronic Communications Regulations 2003, for every marketing email and SMS the Agency sends on its instructions.`);
  L.push(``);
  L.push(`9. LIABILITY`);
  ...
  L.push(`9.2 The Agency is not liable for matters outside its control: advertising-platform decisions, account restrictions, algorithm or policy changes, platform outages, tracking limitations, stock or fulfilment problems, or the Client's own pricing and product decisions.`);
  L.push(``);
...
/* ── Stripe: does Stripe hold a paid Checkout Session for this token? Only when a key is
   set (a restricted key with read on Checkout Sessions is enough). The result is reported
   to the CRM, which does the trusted write. ──────────────────────────────── */
async function stripePaid(token) {
  const key = process.env.STRIPE_SECRET_KEY; if (!key) return { checked: false };
  try {
    const r = await fetch('https://api.stripe.com/v1/checkout/sessions?limit=100', { headers: { Authorization: `Bearer ${key}` } });
    ...
    return { checked: true, paid: true, session: hit.id, customer: hit.customer || '', subscription: hit.subscription || '', email: (hit.customer_details && hit.customer_details.email) || '' };
...
export default async function handler(req, res) {
  ...
  try {
    if (op === 'get' && req.method === 'GET') {
      const row = await rpc('onboard_get', { tok: token });
    ...
    if (op === 'sign' && req.method === 'POST') {
      const row = await rpc('onboard_peek', { tok: token });
      if (!row) return res.status(404).json({ error: 'This link is not valid any more.' });
    ...
    if (op === 'status' && req.method === 'GET') {
      ...
      if (row.status === 'signed') { const s = await stripePaid(token); return res.status(200).json({ ...view, stripeChecked: !!s.checked, stripePaid: !!s.paid, stripe: s.paid ? { session: s.session, customer: s.customer, subscription: s.subscription, email: s.email } : null, stripeError: s.error || null }); }
      return res.status(200).json(view);
    }
